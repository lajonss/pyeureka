...
class EurekaClient:
    ...
    def register(self):
        ...
        self._request('POST', request_uri, 'registration', 204, payload=self.instance_definition)
    def deregister(self):
        self._request('DELETE', comment='deregistration')
    def heartbeat(self):
        self._request('PUT', comment='heartbeat', errors={404: EurekaInstanceDoesNotExistException})
    ...
    def update_metadata(self, key, value):
        request_uri = self._instance_uri, + '/metadata?{}={}'.format(key, value)
    ...
    def _request(self, method, uri=None, comment='operation', accepted_code=200, errors=None, payload=None):
        ...
        request = c.EUREKA_REQUESTS[method](uri, headers=c.EUREKA_HEADERS[method], json=payload)
